...
AddEventHandler('rex-notes:server:saveProp', function(data, propId, citizenid, proptype, title, note, imageUrl)
    ...
    MySQL.Async.execute('INSERT INTO rex_notes (properties, propid, citizenid, proptype, title, note, imageurl) VALUES (@properties, @propid, @citizenid, @proptype, @title, @note, @imageurl)',
    {
        ['@properties'] = datas,
        ['@propid'] = propId,
        ['@citizenid'] = citizenid,
...
AddEventHandler('rex-notes:server:copynote', function(data)
    ...
    MySQL.Async.fetchAll('SELECT title, note, imageurl FROM rex_notes WHERE propid = ?', { data.noteid }, function(result)
        if result and #result > 0 then
            local title = result[1].title
            local content = result[1].note
            local imageUrl = result[1].imageurl
...
AddEventHandler('rex-notes:server:editnote', function(noteid, newTitle, newContent, newImageUrl)
    ...
    MySQL.Async.execute(
        "UPDATE rex_notes SET title = @title, note = @note, imageurl = @imageurl WHERE propid = @propid",
        {
            ['@title'] = newTitle,
            ['@note'] = newContent,
            ['@imageurl'] = newImageUrl or nil,
            ['@propid'] = noteid
        }
    )
